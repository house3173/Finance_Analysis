...
# list_company = ['vic'] 
# list_company = ['hdc', 'nlg', 'hdg', 'vpi', 'kos', 'agg', 'ntl', 'khg', 'ijc', 'hhs', 'dxs', 'scr', 'l14'] 
# list_company = ['bsr', 'gas', 'plx', 'pvs', 'oil', 'pvd', 'psh', 'cng', 'pvb', 'pvc', 'pvg'] 
list_company = ['mwg', 'pnj', 'dgw', 'frt', 'pet']
for company_code in list_company:
    ...
    # list_index_url = ["2019/1/0/0", "2020/1/0/0"]
    excel_path = f"kqhdkd/Ket_qua_hoat_dong_kinh_doanh_{company_code_upper}.xlsx"
    # excel_path = f"bctc/Can_doi_ke_toan_{company_code_upper}.xlsx"
    df_final = pd.DataFrame()

    for ind, index_url in enumerate(list_index_url, 1):
        url = "https://s.cafef.vn/bao-cao-tai-chinh/" + company_code + "/incsta/" + index_url + "/ket-qua-hoat-dong-kinh-doanh-.chn"
    ...
    df_final = pd.read_excel(excel_path, header=None)
    df_final = df_final.dropna(how='all')

    # Thêm hàng đầu tiên làm tiêu đề
    # Tạo DataFrame cho hàng tiêu đề
    header_row = pd.DataFrame([["Quý", "Quý 2 - 2018", "Quý 3 - 2018", "Quý 4 - 2018", "Quý 1 - 2019", "Quý 2 - 2019", "Quý 3 - 2019", "Quý 4 - 2019", "Quý 1 - 2020", "Quý 2 - 2020", "Quý 3 - 2020", "Quý 4 - 2020", "Quý 1 - 2021", "Quý 2 - 2021", "Quý 3 - 2021", "Quý 4 - 2021", "Quý 1 - 2022", "Quý 2 - 2022", "Quý 3 - 2022", "Quý 4 - 2022", "Quý 1 - 2023", "Quý 2 - 2023", "Quý 3 - 2023", "Quý 4 - 2023", "Quý 1 - 2024"]])

    # Chèn hàng tiêu đề vào vị trí thứ 2 (index = 1)
    df_final = pd.concat([df_final.iloc[:0], header_row, df_final.iloc[0:]]).reset_index(drop=True)
    ...
    # df_final.loc[0] = ["Quý", "Quý 2 - 2018", "Quý 3 - 2018", "Quý 4 - 2018", "Quý 1 - 2019"] + [""] * (df_final.shape[1] - 5)
    ...
    # Xóa hàng dữ liệu thứ 3 
    df_final = df_final.drop(1)

    df_final.to_excel(excel_path, index=False)
    print(f"Dữ liệu đã được lưu thành công vào file Excel: {excel_path}")
